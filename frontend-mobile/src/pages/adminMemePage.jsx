...
export default function AdminMemePage() {
  ...
  const [publishedMemes, setPublishedMemes] = useState(
    isCacheValid() ? cache.publishedMemes : [],
  );
  ...
  const [newMemeContext, setNewMemeContext] = useState("");
  ...
  async function handleMemeUpload(event, article) {
    ...
    if (!customTitles[article.id] || !customContexts[article.id]) {
      showToast("Add meme title + context first", "error");
      return;
    }
...
//             "Content-Type": "multipart/form-data",
  ...
  async function handleCreateMeme() {
    if (!newMemeTitle.trim()) {
      showToast("Enter meme title", "error");

      return;
    }

    if (!newMemeContext.trim()) {
      showToast("Enter short context", "error");

      return;
    }

    if (!newMemeImage) {
      showToast("Select image", "error");
...
//             "Content-Type": "multipart/form-data",
      ...
      const memeRes = await axios.post(
        `${BASE_URL}/api/content/memes`,
        {
          title: newMemeTitle,

          shortContext: newMemeContext,
      ...
      setPublishedMemes((prev) => [memeRes.data, ...prev]);
      ...
      <div style={S.page}>
        ...
        <div style={S.statsStrip}>
          <div style={S.statChip}>
            <span style={{ ...S.statNum, color: "#ff4444" }}>
              {candidateCount}
            </span>
          ...
          <div style={S.statChip}>
            <span style={{ ...S.statNum, color: "#22c55e" }}>
              {publishedCount}
            </span>
        ...
        <div style={S.tabBar}>
          <button
            ...
            style={{
              ...S.tab,
              ...(activeTab === "candidates" ? S.tabActive : {}),
            }}
            ...
            {candidateCount > 0 && (
              <span
                style={{
                  ...S.tabBadge,
                  background:
                    activeTab === "candidates" ? "#ff4444" : "#2a2a2a",
                }}
              >
                {candidateCount}
              </span>
          ...
          <button
            ...
            style={{
              ...S.tab,
              ...(activeTab === "published" ? S.tabActive : {}),
            }}
          ...
          >
            ...
            {publishedCount > 0 && (
              <span
                style={{
                  ...S.tabBadge,
                  background: activeTab === "published" ? "#ff4444" : "#2a2a2a",
                }}
              >
                {publishedCount}
              </span>
          ...
          <div style={S.grid}>
            ...
            {articles.map((article) => (
              <CandidateCard
                ...
                onTitleChange={(v) =>
                  setCustomTitles((p) => ({ ...p, [article.id]: v }))
                }
                onContextChange={(v) =>
                  setCustomContexts((p) => ({ ...p, [article.id]: v }))
                }
                onFileSelect={(file) =>
                  setSelectedFiles((p) => ({ ...p, [article.id]: file }))
                }
                onPublish={() =>
                  handleMemeUpload(
                    { target: { files: [selectedFiles[article.id]] } },
                    article,
                  )
                }
              />
          ...
          <div style={S.grid}>
            ...
            {publishedMemes.map((meme) => (
              <PublishedCard
                ...
                onEditToggle={() =>
                  setEditingMemeId(editingMemeId === meme.id ? null : meme.id)
                }
                onUpdate={(updated) =>
                  setPublishedMemes((prev) =>
                    prev.map((m) =>
                      m.id === meme.id ? { ...m, ...updated } : m,
                    ),
                  )
                }
                onDelete={() => {
                  axios
                    .delete(`${BASE_URL}/api/content/memes/${meme.id}`)
                    .then(() => {
                      cache.publishedMemes = (
                        cache.publishedMemes || []
                      ).filter((m) => m.id !== meme.id);
                      setPublishedMemes((prev) =>
                        prev.filter((m) => m.id !== meme.id),
                      );
                      showToast("Meme deleted", "success");
                    })
                    .catch(console.error);
                }}
                onSave={(updated) => {
                  const token = localStorage.getItem("token");
                  axios
                    .put(`${BASE_URL}/api/content/memes/${meme.id}`, updated, {
                      headers: { Authorization: `Bearer ${token}` },
                    })
                    .then(() => {
                      setEditingMemeId(null);
                      showToast("Meme updated!", "success");
                    })
                    .catch(console.error);
                }}
              />
        ...
        >
          ...
          >
            ...
            <textarea
              ...
              onChange={(e) => setNewMemeContext(e.target.value)}
              style={{
                ...S.textArea,
                marginTop: "12px",
              }}
            ...
            >
              ...
              >
                {creatingMeme ? "Publishing..." : "Publish Meme"}
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="meme-card" style={S.card}>
      ...
      <div style={S.cardBody}>
        <div style={S.cardMeta}>
          ...
          <span style={{ ...S.scoreNum, color: scoreColor(score) }}>
        ...
        {selectedFile && <div style={S.fileChip}>✓ {selectedFile.name}</div>}
            ...
            <div style={S.actions}>
              <button
                ...
                onClick={() =>
                  onSave({
                    title: meme.title,
                    shortContext: meme.shortContext,
