...
export default function LoginPage() {
  ...
  const handleVerifyRegOtp = async () => {
    ...
    setError("");
    try {
      const response = await verifyOtp(form.email, regOtp, "REGISTRATION");
  ...
  const handleResetPassword = async (e) => {
    ...
    setError("");
    try {
      await resetPassword(fpEmail, fpOtp, fpNewPass);
      setFpSuccess(true);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === "login") {
        ...
        const authUserId =
          res.data.userId ?? res.data.id ?? res.data.authUserId;
                ...
                <GoogleLogin
                  onSuccess={async (cr) => {
                    try {
                      ...
                      // ← ADD THIS: check onboarding from DB
                      const profileRes = await api.get(
                        `/api/users/profile/${res.data.userId}`,
                        {
                          headers: {
                ...
                {!regOtpVerified && (
                  <button
                    ...
                    style={{
                      ...
                      alignSelf: "stretch",
                      opacity:
                        !form.email || regCooldown > 0 || sendingRegOtp
                          ? 0.5
                          : 1,
                    }}
                  >
              ...
              {form.password && (
                ...
                >
                  <div style={{ display: "flex", gap: "4px" }}>
                    {[0, 1, 2, 3, 4].map((i) => {
                      const strength =
                        5 - validatePassword(form.password).length;
                      const colors = [
                        "#ef4444",
                        "#ef4444",
                        "#f97316",
                    ...
                    {!fpOtpVerified && (
                      <button
                        ...
                        style={{
                          ...
                          alignSelf: "stretch",
                          opacity:
                            !fpEmail || fpCooldown > 0 || sendingFpOtp
                              ? 0.5
                              : 1,
                        }}
                      >
                  ...
                  {fpOtpVerified && (
                    ...
                    >
                      ...
                      <button
                        ...
                        type="submit"
                        disabled={
                          loading || !fpNewPass || fpNewPass !== fpConfirmPass
                        }
                        style={{
                          ...S.submitBtn,
                          opacity:
                            !fpNewPass || fpNewPass !== fpConfirmPass ? 0.5 : 1,
                        }}
                      >
                        {loading ? (
                          <span style={S.spinner} />
...
const S = {
  ...
  meshBg: {
    ...
    pointerEvents: "none",
    background:
      "radial-gradient(ellipse at 20% 20%, rgba(255,106,0,0.07) 0%, transparent 55%), radial-gradient(ellipse at 80% 80%, rgba(255,106,0,0.05) 0%, transparent 55%)",
  },
  bgWordmark: {
    position: "fixed",
    top: "50%",
  ...
  card: {
    ...
    boxShadow:
      "0 40px 80px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,106,0,0.07)",
  },
  cardTopLine: {
    position: "absolute",
    top: 0,
    ...
    height: "1px",
    background:
      "linear-gradient(90deg, transparent, rgba(255,106,0,0.7), transparent)",
    borderRadius: "999px",
  },
  logoWrap: {
    display: "flex",
  ...
  submitBtn: {
    ...
    marginTop: "4px",
    background:
      "linear-gradient(135deg, #FF6A00 0%, #cc5200 50%, #FF6A00 100%)",
