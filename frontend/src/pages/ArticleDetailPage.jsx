...
const BADGE_CONFIG = {
    NEWS_HUNTER_I:      { label: "News Hunter",       tier: "I",   emoji: "🗞️",  color: "#f59e0b" },
    NEWS_HUNTER_II:     { label: "News Hunter",       tier: "II",  emoji: "🗞️",  color: "#f97316" },
    NEWS_HUNTER_III:    { label: "News Hunter",       tier: "III", emoji: "🗞️",  color: "#ef4444" },
    SIGNAL_BOOSTER_I:   { label: "Signal Booster",   tier: "I",   emoji: "📡",  color: "#3b82f6" },
    SIGNAL_BOOSTER_II:  { label: "Signal Booster",   tier: "II",  emoji: "📡",  color: "#6366f1" },
    SIGNAL_BOOSTER_III: { label: "Signal Booster",   tier: "III", emoji: "📡",  color: "#8b5cf6" },
    DEBATE_LORD_I:      { label: "Debate Lord",       tier: "I",   emoji: "🎤",  color: "#10b981" },
    DEBATE_LORD_II:     { label: "Debate Lord",       tier: "II",  emoji: "🎤",  color: "#059669" },
    DEBATE_LORD_III:    { label: "Debate Lord",       tier: "III", emoji: "🎤",  color: "#047857" },
};
...
const FRAME_STYLES = {
    ...
    orange_pulse: { border: "3px solid #f97316", boxShadow: "0 0 16px #f97316", animation: "pulse 2s infinite" },
    fire:         { border: "3px solid #ef4444", boxShadow: "0 0 24px #ef4444, 0 0 48px #f97316" },
    galaxy:       { border: "3px solid #8b5cf6", boxShadow: "0 0 24px #8b5cf6, 0 0 48px #3b82f6, 0 0 72px #ec4899" },
};
...
export default function ArticleDetailPage() {
    ...
    useEffect(() => {
        ...
        axios.post(
            `${BASE_URL}/api/users/track/referral`,
            {
                referrerId,
                contentId
            }
    ...
    useEffect(() => {
        const random = VIBE_STRINGS[Math.floor(Math.random() * VIBE_STRINGS.length)];
        setVibeText(random);
    }, []);
        ...
        timerRef.current = setTimeout(() => {
            ...
            axios
                ...
                .then((res) => {
                    ...
                    const newVibe = VIBE_STRINGS[Math.floor(Math.random() * VIBE_STRINGS.length)];
                    setVibeText(newVibe);
                })
                .catch(console.error);
        }, 5000); // 5 seconds
    ...
    async function handleShare() {
        try {
            ...
            if (authUserId) {
                ...
                axios
                    .post(`${BASE_URL}/api/content/${contentId}/share`, {}, {
                        headers: { Authorization: `Bearer ${token}` },
                    })
                    .catch(console.error);
            }
