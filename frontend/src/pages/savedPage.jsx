...
export default function SavedPage() {
    ...
    useEffect(() => {
        ...
        if (!authUserId || !token) { navigate("/login"); return; }
    ...
    console.log('SAVED ARTICLES:', savedArticles.map(a => a.id)); // ← add
            ...
            // Seed save cache
            const map = {};
            savedArticles.forEach(a => { map[a.id] = true; });
            setSavedCache(map);


            const savedMemeIds =
                new Set(profileRes.data.savedMemeIds || []);

            const savedMemes =
                allMemes.filter(
                    meme => savedMemeIds.has(meme.id)
                );
            setMemes(savedMemes);
                                        ...
                                        <div style={S.memeImgWrap}>
                                            <img
                                                ...
                                                onError={e => {
                                                    ...
                                                    e.target.src = 'https://placehold.co/400x400/111/fff?text=Meme';
                                                }}
                                            />
                                        </div>
