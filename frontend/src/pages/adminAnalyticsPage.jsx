...
function BarChart({ data, xKey, yKey, color = "#FF6A00", height = 160, label = "" }) {
    if (!data || data.length === 0) return (
        <div style={C.emptyChart}>No data yet</div>
    );
    const max = Math.max(...data.map(d => Number(d[yKey]) || 0), 1);
    return (
        <div style={{ width: "100%" }}>
            {label && <div style={C.chartLabel}>{label}</div>}
            <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", height: `${height}px`, width: "100%", overflowX: "auto" }}>
                {data.map((d, i) => {
                    const val = Number(d[yKey]) || 0;
                    const pct = max > 0 ? (val / max) * 100 : 0;
                    return (
                        <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, minWidth: "28px", gap: "4px" }}>
                            <div style={{ fontSize: "9px", color: "#555", fontWeight: "600" }}>{val}</div>
                            <div style={{
                            ...
                            <div style={{
                                ...
                                maxWidth: "40px", overflow: "hidden", textOverflow: "ellipsis",
                            }}>
                                {String(d[xKey]).length > 6 ? String(d[xKey]).slice(-5) : d[xKey]}
                            </div>
                        </div>
                    );
                })}
...
export default function AdminAnalyticsPage() {
    ...
    async function fetchAll() {
        ...
        try {
            const [
                ...
                axios.get(`${BASE_URL}/api/analytics/engagement/daily?period=14`, auth()),
                ...
                axios.get(`${BASE_URL}/api/analytics/content/top-articles?limit=10`, auth()),
            ...
            setError("Failed to load analytics. Make sure you are authenticated as ADMIN.");
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const engagementArr = Object.values(engagementByDate).sort((a,b) => a.date > b.date ? 1 : -1);
    ...
    if (error) return (
        <div style={C.loadingPage}>
            ...
            <div style={{ color: "#ef4444", fontSize: "14px", textAlign: "center", maxWidth: "300px" }}>{error}</div>
            <button onClick={fetchAll} style={C.retryBtn}>Retry</button>
        </div>
    );
            ...
            <div style={C.page}>
                ...
                <div style={C.topBar}>
                    ...
                    <div style={C.topBarRight}>
                        ...
                        <button className="refresh-btn" style={C.refreshBtn} onClick={fetchAll}>↺ Refresh</button>
                        <button style={C.backBtn} onClick={() => navigate("/")}>← Home</button>
                    </div>
                </div>
                    ...
                    {tab === "overview" && overview && (
                        <div style={{ animation: "fadeUp 0.4s ease" }}>
                            ...
                            <div style={C.statsGrid}>
                                <StatCard label="Total Reads"     value={overview.totalReads}        icon="📖" color="#FF6A00" delay={0}   />
                                <StatCard label="Total Shares"    value={overview.totalShares}       icon="↗️"  color="#3b82f6" delay={60}  />
                                <StatCard label="Total Comments"  value={overview.totalComments}     icon="💬" color="#22c55e" delay={120} />
                                <StatCard label="Total Users"     value={overview.totalUsers}        icon="👥" color="#a855f7" delay={180} />
                                <StatCard label="New Today"       value={overview.newUsersToday}     icon="🌅" color="#f59e0b" delay={240} sub="new users" />
                                <StatCard label="New This Week"   value={overview.newUsersThisWeek}  icon="📅" color="#ec4899" delay={300} sub="new users" />
                                <StatCard
                                    label="Today's Active Users"
                                    value={Array.isArray(dau) && dau.length > 0
                                            ...
                                            <tbody>
                                                {engagementArr.map((row, i) => (
                                                    <tr key={i} className="article-row" style={C.tr}>
                                                        ...
                                                        <td style={{...C.td, color:"#FF6A00", fontWeight:"700"}}>{row.reads}</td>
                                                        <td style={{...C.td, color:"#3b82f6", fontWeight:"700"}}>{row.shares}</td>
                                                        <td style={{...C.td, color:"#22c55e", fontWeight:"700"}}>{row.comments}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                        ...
                        <Section title="Article Click-Through Rate" icon="🎯">
                            <div style={C.twoCol}>
                                <div style={C.infoCard}>
                                    ...
                                    <div style={{...C.infoValue, color:"#888"}}>{ctr.impressions}</div>
                                </div>
                                <div style={C.infoCard}>
                                    <div style={C.infoLabel}>Clicks</div>
                                    <div style={{...C.infoValue, color:"#3b82f6"}}>{ctr.clicks}</div>
                                </div>
                                <div style={C.infoCard}>
                                    <div style={C.infoLabel}>CTR</div>
                                    <div style={{...C.infoValue, color:"#FF6A00"}}>{ctr.ctr}</div>
                                </div>
                            </div>
                        </Section>
                    )}
                                    ...
                                    <table style={C.table}>
                                        ...
                                        <tbody>
                                            {byCategory.map((row, i) => {
                                                const total = byCategory.reduce((s, r) => s + Number(r.reads), 0);
                                                const pct = total > 0 ? ((Number(row.reads)/total)*100).toFixed(1) : "0.0";
                                                return (
                                                    <tr key={i} className="article-row" style={C.tr}>
                                                        <td style={C.td}>
                                        ...
                                        <table style={C.table}>
                                            ...
                                            <tbody>
                                                {topArticles.map((row, i) => (
                                                    <tr key={i} className="article-row" style={C.tr}>
                                                        <td style={{...C.td, color:"#555", fontFamily:"'DM Mono',monospace"}}>
                                                            {i + 1}
                                                        </td>
                                                        <td style={{...C.td, fontFamily:"'DM Mono',monospace", color:"#888"}}>
                                                            #{row.contentId}
                                                        </td>
                                                        <td style={C.td}>
                                                            <span style={C.catPill}>{row.category || "—"}</span>
                                                        </td>
                                                        <td style={{...C.td, color:"#FF6A00", fontWeight:"700"}}>
                                                            {row.reads}
                                                        </td>
                                                    </tr>
                                                ))}
                                            ...
                                            <table style={C.table}>
                                                ...
                                                <tbody>
                                                    {userGrowth.map((row, i) => (
                                                        <tr key={i} className="article-row" style={C.tr}>
                                                            <td style={C.td}>{row.date}</td>
                                                            <td style={{...C.td, color:"#a855f7", fontWeight:"700"}}>{row.count}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                ...
                                <Section title="User Retention" icon="🔁">
                                    <div style={C.twoCol}>
                                        <div style={C.infoCard}>
                                            ...
                                            <div style={{...C.infoValue, color:"#22c55e"}}>{retention.day1Rate}</div>
                                            <div style={{fontSize:"11px", color:"#444", marginTop:"4px"}}>{retention.day1Retained} users</div>
                                        </div>
                                        <div style={C.infoCard}>
                                            <div style={C.infoLabel}>Day 7 Retention</div>
                                            <div style={{...C.infoValue, color:"#3b82f6"}}>{retention.day7Rate}</div>
                                            <div style={{fontSize:"11px", color:"#444", marginTop:"4px"}}>{retention.day7Retained} users</div>
                                        </div>
                                        <div style={C.infoCard}>
                                            <div style={C.infoLabel}>Day 30 Retention</div>
                                            <div style={{...C.infoValue, color:"#a855f7"}}>{retention.day30Rate}</div>
                                            <div style={{fontSize:"11px", color:"#444", marginTop:"4px"}}>{retention.day30Retained} users</div>
                                        </div>
                                        <div style={C.infoCard}>
                                            <div style={C.infoLabel}>Total Users</div>
                                            <div style={{...C.infoValue, color:"#FF6A00"}}>{retention.totalUsers}</div>
                                        </div>
                                    </div>
                                </Section>
                            )}
                                    ...
                                    <div style={C.twoCol}>
                                        <div style={C.infoCard}>
                                            ...
                                            <div style={{fontSize:"11px", color:"#444", marginTop:"4px"}}>
                                                {Math.round((sessionStats.avgSessionSeconds || 0) / 60)}m avg
                                            </div>
                                        </div>
                                        <div style={C.infoCard}>
                                ...
                                <Section title="User Stats" icon="📊">
                                    <div style={C.twoCol}>
                                        <div style={C.infoCard}>
                                            ...
                                            <div style={{...C.infoValue, color:"#a855f7"}}>{overview.totalUsers}</div>
                                        </div>
                                        <div style={C.infoCard}>
                                            <div style={C.infoLabel}>Joined Today</div>
                                            <div style={{...C.infoValue, color:"#f59e0b"}}>{overview.newUsersToday}</div>
                                        </div>
                                        <div style={C.infoCard}>
                                            <div style={C.infoLabel}>Joined This Week</div>
                                            <div style={{...C.infoValue, color:"#22c55e"}}>{overview.newUsersThisWeek}</div>
                                        </div>
                                        <div style={C.infoCard}>
                                            <div style={C.infoLabel}>Total Reads</div>
                                            <div style={{...C.infoValue, color:"#FF6A00"}}>{overview.totalReads}</div>
                                        </div>
                                    </div>
                                </Section>
                            )}
...
const C = {
    ...
    liveText: { color: "#22c55e", fontSize: "11px", fontWeight: "700", letterSpacing: "0.1em" },
    refreshBtn: {
        background: "rgba(255,106,0,0.1)",
        border: "1px solid rgba(255,106,0,0.2)",
        color: "#FF6A00", padding: "6px 12px",
    ...
    statLabel: { color: "#D4D4D4", fontSize: "11px", fontWeight: "600", letterSpacing: "0.08em", textTransform: "uppercase" },
    ...
    chartTitle: {
        ...
        color: "#E5E5E5", letterSpacing: "0.06em",
    ...
    // Loading
    loadingPage: {
        background: "#0B0B0B", height: "100vh",
        display: "flex", flexDirection: "column",
        alignItems: "center", justifyContent: "center",
        gap: "16px", fontFamily: "'Space Grotesk',sans-serif",
    },
    loadingRing: {
    ...
    retryBtn: {
        ...
        background: "rgba(255,106,0,0.1)",
        border: "1px solid rgba(255,106,0,0.3)",
        color: "#FF6A00", padding: "10px 24px",
        borderRadius: "8px", cursor: "pointer",
