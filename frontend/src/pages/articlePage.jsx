...
const FALLBACK_IMAGE = 'https://placehold.co/800x400/1a1a1a/FF6A00?text=SqueezeFeed';
const VIBES = [
...
export default function ArticlePage() {
        ...
        const timer = setTimeout(() => {
            ...
            if (isPublic && refUser) {
                ...
                if (!localStorage.getItem(shareKey)) {
                    axios.post(`http://localhost:8080/api/users/track/share/${refUser}`)
            ...
            if (authUserId) {
                axios.post(`http://localhost:8080/api/users/track/read/${authUserId}`)
                    .catch(console.error);
            }
        }, 6000);
    ...
    const handleComment = async () => {
        ...
        try {
            const res = await createComment({
                contentId: Number(id), authUserId: Number(authUserId),
    ...
    const handleShare = () => {
        const authUserId = localStorage.getItem("authUserId");
        const shareUrl   = `${window.location.origin}/public/article/${id}?ref=${authUserId}`;
    ...
    const title   = decodeHTML(article.translatedTitle?.[selectedLanguage] || article.title);
    const summary = decodeHTML(article.translatedSummary?.[selectedLanguage] || article.summary)
                    || decodeHTML(article.description) || 'No summary available.';
        ...
        <>
            ...
            <div style={S.page}>
                ...
                <div style={S.container}>
                    ...
                    {!isPublic && (
                        <button className="sf-back" style={S.backBtn} onClick={() => navigate(-1)}>
                            ← Back
                        </button>
                    )}
                    ...
                    <article style={S.article}>
                       ...
                       {!isPublic && (
                           <div style={S.meta}>
                               ...
                               <button className="sf-share" style={S.shareBtn} onClick={handleShare}>
                                   {copied ? '✓ Copied!' : '↗ Share'}
                               </button>
                           </div>
                       )}
...
{/*                         )} */}
                    ...
                    <div style={S.commentsSection}>
                        ...
                        <div style={S.commentsList}>
                            {comments.length === 0 ? (
                                <p style={{ color: '#444', fontSize: '14px', textAlign: 'center', padding: '24px 0' }}>
                    ...
                    {isPublic && (
                        <div style={S.ctaBanner}>
                            <div style={S.ctaIcon}><SqueezeMark size={32} /></div>
                            <div>
...
const S = {
    ...
    title: {
        fontFamily: "'Playfair Display', serif",
        color: '#f0ece0', fontSize: 'clamp(22px, 5vw, 32px)',
    ...
    commentCount: {
        background: 'rgba(255,106,0,0.12)',
        color: '#FF6A00', fontSize: '12px', fontWeight: '700',
    ...
    commentInput: {
        ...
        borderRadius: '14px', color: '#e0dbd0',
    ...
    commentUser: {
        color: '#FF6A00', fontWeight: '700',
        fontSize: '12px', letterSpacing: '0.04em',
        marginBottom: '6px',
    },
    commentText: {
        color: '#777', lineHeight: '1.55', fontSize: '14px',
    },
    ...
    ctaSub: {
        color: '#555', fontSize: '13px', lineHeight: '1.4',
    },
    ctaBtn: {
        marginLeft: 'auto',
        background: 'linear-gradient(135deg, #FF6A00, #cc5200)',
