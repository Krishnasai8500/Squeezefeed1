...
export default function UserSearchPage() {
    ...
    const handleSearch = (value) => { setQuery(value); setActivePill(""); fetchResults(value); };
    const handlePill   = (label, term) => { setActivePill(label); setQuery(term); fetchResults(term); };
    const clearSearch  = () => { setQuery(""); setResults([]); setActivePill(""); inputRef.current?.focus(); };
            ...
            <div style={S.page}>
                ...
                <div style={S.container}>
                    ...
                    <div style={S.header}>
                        ...
                        <div style={S.logoRow}>
                            <SqueezeMark size={16} />
                            <span style={S.logoText}>Squeeze<span style={{color:'#FF6A00'}}>Feed</span></span>
                        </div>
                    </div>
                    ...
                    <div style={{
                        ...S.searchWrap,
                        boxShadow: focused ? '0 0 0 2px #FF6A00' : '0 0 0 1px rgba(255,255,255,0.07)',
                    }}>
                        <span style={{ position:'absolute', left:'16px', top:'50%', transform:'translateY(-50%)', fontSize:'16px', opacity: focused ? 1 : 0.4 }}>
                            🔍
                        </span>
                        <input
                            ref={inputRef}
                            ...
                            {[1,2,3].map(i => (
                                <div key={i} style={S.shimmerCard}>
                                    <div style={{
                                        ...
                                        backgroundImage: SHIMMER, backgroundSize:'800px 100%',
                                        animation:'shimmer 1.4s infinite linear', flexShrink:0,
                                    }}/>
                                    <div style={{ flex:1, display:'flex', flexDirection:'column', gap:'10px' }}>
                                        <div style={{ height:'14px', borderRadius:'6px', width:'50%', backgroundImage:SHIMMER, backgroundSize:'800px 100%', animation:'shimmer 1.4s infinite linear' }}/>
                                        <div style={{ height:'11px', borderRadius:'6px', width:'30%', backgroundImage:SHIMMER, backgroundSize:'800px 100%', animation:'shimmer 1.4s infinite linear 0.2s' }}/>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                            ...
                            {query && results.length === 0 && (
                                <div style={S.empty}>
                                    ...
                                    <div style={{ color:'#555', fontSize:'16px', fontWeight:'700' }}>No users found</div>
                                    <div style={{ color:'#333', fontSize:'12px', marginTop:'6px' }}>Try a different search</div>
                                </div>
                            )}
                        </div>
                    )}
...
const S = {
    ...
    searchWrap: {
        ...
        marginBottom:'14px', transition:'box-shadow 0.2s',
    },
    input: {
    ...
    clearBtn: {
        ...
        border:'none', color:'#777', fontSize:'11px',
        cursor:'pointer', display:'flex',
        alignItems:'center', justifyContent:'center',
    },
    pillsRow: { display:'flex', gap:'8px', flexWrap:'wrap', marginBottom:'20px' },
    pill: {
    ...
    fullName: {
        fontFamily:"'Space Grotesk',sans-serif",
        fontSize:'15px', fontWeight:'700', color:'#f0ece0',
        letterSpacing:'-0.2px',
    },
    userName: { color:'#FF6A00', fontSize:'12px', marginTop:'2px', fontWeight:'600' },
    badgePill: {
        background:'rgba(255,106,0,0.1)',
        border:'1px solid rgba(255,106,0,0.2)',
        color:'#FF6A00', padding:'4px 10px',
