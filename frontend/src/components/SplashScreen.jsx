...
export default function SplashScreen({ onDone }) {
    ...
    useEffect(() => {
        ...
        // ── Scene elements ────────────────────────────────────
        let gridEl, scanEl, cornerEls = [], ringEls = [], barEls = [], dotEl;
        ...
        function buildCorners() {
            ...
            cornerEls = positions.map(pos => {
                const css = Object.entries({
                    ...
                    borderColor:'rgba(255,106,0,0.7)', opacity:0,
                    transition:'all 0.4s',
                    ...pos,
        ...
        function buildBars() {
            const defs = [
                { w:62, yOff:-95 }, { w:48, yOff:-66 }, { w:36, yOff:-37 },
                { w:24, yOff:-8  }, { w:14, yOff:21   },
            ];
            barEls = defs.map(d => makeEl(`
                position:absolute;
        ...
        async function sceneGrid() {
            ...
            await animate(1200, (t, e) => {
                ...
                if (Math.random() > 0.6) spawnTrail(Math.random() * W(), parseFloat(scanEl.style.top) / 100 * H());
            });
            scanEl.style.opacity = '0';

            // Corner brackets snap in
            await Promise.all(cornerEls.map((el, i) =>
                new Promise(res => setTimeout(() => {
                    animate(300, (t, e) => { el.style.opacity = String(e); }).then(res);
                }, i * 60))
            ));
            await wait(200);
        }
        ...
        async function sceneBars() {
            ...
            ringEls.forEach((r, i) => {
                setTimeout(() => {
                    animate(400, (t, e) => { r.style.opacity = String(e * 1); });
                }, i * 100);
            });
            await wait(300);
            ...
            for (let i = 0; i < barEls.length; i++) {
                ...
                spawnBurst(rect.right - sr.left, rect.top - sr.top + rect.height/2,
                    8, 3, 20, 2, [ORG, '#fff', ORG2]);
                await wait(40);
            }
        ...
        async function sceneExplode() {
            ...
            // Mega burst
            for (let i = 0; i < 3; i++) {
                spawnBurst(cx, cy, 40, 8 + i*3, 50 + i*10, 3, [ORG, '#fff', ORG2, '#ffcc00']);
            }
    ...
    return (
        <div ref={splashRef} style={{
            ...
            <div style={{
                ...
                background: 'radial-gradient(ellipse 60% 40% at 50% 50%, rgba(255,106,0,0.08) 0%, transparent 70%)',
            }} />
