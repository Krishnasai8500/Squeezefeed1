...
const BURST_EMOJIS = ['🔥','💀','🫡','⚡','🌀','👁️','🎯','💥','🧠','📲','🗞️','🎬','🌍','💉','😭','🏆','🤯','⚠️'];
let emojiCounter = 0;

// ── Typing hook ───────────────────────────────────────────────
function useTypingEffect(strings, typingSpeed = 45, deleteSpeed = 22, pauseMs = 2800) {
...
export default function Navbar({ onRefresh }) {
    ...
    useEffect(() => {
        const handler = () =>
            setHasBadgeNotification(localStorage.getItem("hasPendingBadges") === "true");
    ...
    const handleVibeClick = useCallback((e) => {
        ...
        setEmojiBursts(prev => [...prev, { id, emoji, x: e.clientX, y: e.clientY }]);
        setTimeout(() => setEmojiBursts(prev => prev.filter(b => b.id !== id)), 900);
    }, []);
        ...
        <>
            ...
            {emojiBursts.map(burst => (
                <div key={burst.id} style={{
                    ...
                    fontSize: '28px', pointerEvents: 'none', zIndex: 99999,
                    animation: 'emojiFly 0.9s ease-out forwards', userSelect: 'none',
                }}>
                    {burst.emoji}
                </div>
            ))}
                    ...
                    <div style={S.container}>
                       <div
                           ...
                           onClick={() => {
                               if (onRefresh) {
                                   ...
                                   window.dispatchEvent(new CustomEvent("refreshCurrentPage", {
                       ...
                       >
                           <svg width="20" height="25" viewBox="0 0 64 80" fill="none">
                               ...
                               <rect x="10" y="19" width="44" height="7" rx="3.5" fill="#FF6A00" opacity="0.85"/>
                               <rect x="18" y="32" width="28" height="7" rx="3.5" fill="#FF6A00" opacity="0.65"/>
                               <rect x="24" y="45" width="16" height="7" rx="3.5" fill="#FF6A00" opacity="0.45"/>
                               <rect x="28" y="56" width="8"  height="7" rx="3.5" fill="#FF6A00" opacity="0.3"/>
                        ...
                        <div style={S.pills}>
                            <span className="nav-pill" style={S.pill} onClick={() => navigate('/memes')}>
                                🎨 Visual
                            </span>
                            {role === 'ADMIN' && (
                                <>
                                    <span className="nav-pill" style={S.pill} onClick={() => navigate('/admin/memes')}>🛠 Memes</span>
                                    <span className="nav-pill" style={S.pill} onClick={() => navigate('/admin/news')}>📰 News</span>
                                    <span className="nav-pill" style={S.pill} onClick={() => navigate('/admin/analytics')}>
                                            📊 Analytics
                                        </span>
                                    <span className="nav-pill" style={S.pill} onClick={() => navigate('/admin/reports')}>
                                        🚩 Reports
                                    </span>
                                </>
                            )}
...
const S = {
    ...
    vibeBar: {
        width: '80%',
        background: 'linear-gradient(90deg,#cc5200,#FF6A00,#ff8c3a,#FF6A00,#cc5200)',
...
export function SqueezeMark({ size = 48, color = "#FF6A00", darkMode = true }) {
    ...
    return (
        <svg width={size} height={h} viewBox="0 0 64 80" fill="none">
            ...
            <rect x="10" y="19" width="44" height="7" rx="3.5" fill={color} opacity="0.85" />
            <rect x="18" y="32" width="28" height="7" rx="3.5" fill={color} opacity="0.65" />
            <rect x="24" y="45" width="16" height="7" rx="3.5" fill={color} opacity="0.45" />
            <rect x="28" y="56" width="8"  height="7" rx="3.5" fill={color} opacity="0.3"  />
